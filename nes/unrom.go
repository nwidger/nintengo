...
type BankSelectFlag uint8

const (
	PRGBank BankSelectFlag = 1 << iota
)
...
func (unrom *UNROM) Fetch(address uint16) (value uint8) {
	...
	case address >= 0x8000 && address <= 0xffff:
		...
		case address >= 0x8000 && address <= 0xbfff:
			if unrom.ROMFile.prgBanks > 0 {
				value = unrom.ROMFile.romBanks[unrom.BankSelect][index]
			}
		// PRG bank 2
...
func (unrom *UNROM) Store(address uint16, value uint8) (oldValue uint8) {
	...
	// PRG banks 1 & 2
	case address >= 0x8000 && address <= 0xffff:
		unrom.BankSelect = value & 0x07
	}

	return
}

func (unrom *UNROM) Mirrors() (mirrors map[uint32]uint32) {
	return unrom.ROMFile.Mirrors()
}
